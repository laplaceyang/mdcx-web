...
from mdcx.core.scraper import again_search, get_resume_state, start_new_scrape
...
from mdcx.controllers.main_window.preview_loader import PreviewImageLoader
...
class MyMAinWindow(QMainWindow):
    ...
    def pushButton_start_scrape_clicked(self):
        ...
        if text == "开始":
            if not self._confirm_resume_scrape():
                start_new_scrape(FileMode.Default)
        elif text == "■ 停止":
            self.pushButton_stop_scrape_clicked()
        # text == "■ 停止中"：防抖——用户疯狂点击时静默忽略，避免重复触发 stop 流程

    def _confirm_resume_scrape(self) -> bool:
        """断点续刮确认弹窗（决策逻辑在桌面层；核心层只提供 get_resume_state 事实）。

        返回 True 表示本次启动已被处理（已续刮或用户取消），False 表示从头刮削。
        """
        ...
        reply = box.exec()
        if reply == QMessageBox.StandardButton.Yes:
            signal_qt.show_log_text("🍯 🍯 🍯 NOTE: 用户选择继续刮削剩余任务。")
        elif reply == QMessageBox.StandardButton.No:
            signal_qt.show_log_text("🍯 🍯 🍯 NOTE: 用户选择从头刮削。")
            return False  # 从头刮削
        else:
            signal_qt.show_log_text("🍯 🍯 🍯 NOTE: 已取消本次刮削启动。")
            return True  # 不刮削（包括点取消、ESC、右上角关闭）

        if not resume.first_in_scan_dirs:
            box = QMessageBox(
                QMessageBox.Icon.Warning,
                "提醒",
                f"很重要！！请注意：\n当前待刮削目录：{';'.join(resume.scan_dirs)}\n"
                f"剩余任务文件路径：{resume.first_path.resolve()}\n"
            ...
            if reply == QMessageBox.StandardButton.No:
                return True
        start_new_scrape(FileMode.Default, resume.remain_list)
        return True
