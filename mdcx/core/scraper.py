...
class ResumeInfo:
    ...
    原 get_remain_list 的无头化拆分：核心层只负责收集事实（剩余列表、是否在
    当前扫描目录内），“继续/从头/取消”的弹窗决策上移给调用方（桌面弹窗、web 前端）。
    """

    remain_list: list[Path]
    first_path: Path
    first_in_scan_dirs: bool
    scan_dirs: list[str]


def get_resume_state() -> ResumeInfo | None:
    """读取上次未完成任务（remain.txt）。无剩余任务或未开启续刮开关时返回 None。

    与旧版 get_remain_list 一致：读取成功即刷新 Flags.remain_list（无论是否可续刮）。
    """
    ...
    first = Flags.remain_list[0]
    return ResumeInfo(
        remain_list=Flags.remain_list,
        first_path=first,
        first_in_scan_dirs=is_any_descendant(first, *movie_paths),
        scan_dirs=[str(path) for path in movie_paths],
    )


def start_resume_scrape(remain_list: list[Path]) -> None:
    """按剩余任务列表续刮（调用方在用户确认“继续刮削剩余任务”后调用）。"""
